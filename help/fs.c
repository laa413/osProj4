...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NINDIRECT*NINDIRECT){
        ...
        bp = bread(ip->dev, addr);
        a = (uint*)bp->data;
        if ((addr = a[bn/(NINDIRECT)]) == 0) { // get index for 1st level                        
              a[bn/(NINDIRECT)] = addr = balloc(ip->dev);
              log_write(bp);
        }
        brelse(bp);               // release the first level
        bp = bread(ip->dev, addr);
        a = (uint*)bp->data;
        if ((addr = a[bn%(NINDIRECT)]) == 0) { //  get index for 2nd level 
             a[bn%(NINDIRECT)] = addr = balloc(ip->dev);
             log_write(bp);
        }
        brelse(bp); //release the second level
        return addr;
    }
